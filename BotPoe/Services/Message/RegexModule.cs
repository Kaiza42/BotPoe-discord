...
public async Task ShowRegexCodes()
{
    ...
    public async Task BuildRegex(string name, params string[] codes)
    {
        ...
        if (patterns.Any())
        {
            ...
            _regexService.SaveToHistory(Context.User.Id, name, finalRegex, string.Join(", ", foundCodes));
        ...
        {
            await ReplyAsync("Erreur : Aucun code valide detecte. Tapez !rgxMap pour la liste.");
        }
    }
    ...
    {
        var history = _regexService.GetUserHistory(Context.User.Id);
